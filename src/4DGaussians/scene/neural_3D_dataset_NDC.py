...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        self.image_paths, self.image_poses, self.image_times, N_cam, N_time, self.cam_ids = self.load_images_path(videos, self.split)
        self.cam_number = N_cam # total number of cameras
        self.time_number = N_time # total number of time
    ...
    def load_images_path(self,videos,split):
        ...
        for index, video_path in enumerate(videos):
            ...
            count = 0
            video_images_path = video_path.split('.')[0]
            cam_id = video_images_path.split('/')[-1]
            image_path = os.path.join(video_images_path,"images")
            video_frames = cv2.VideoCapture(video_path)
