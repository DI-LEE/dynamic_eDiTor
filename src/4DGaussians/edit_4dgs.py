...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    if stage == "edit":
        ...
        # Record start time (removed time reporting)
        ...
        for i, windows in enumerate[list[tuple[int, int, Any]]](tqdm(generate_subgrid(grid), desc=description, total=total_windows)):
            ...
            with torch.inference_mode():
                edited_output, right_cache, bottom_cache = qwenimage_pipeline_ours(
                    Editing_module,
                    image=batch_images,
                    prompt=prompts,
                    generator=generator,
                    ...
                    prev_bottom_cache=prev_bottom_cache if bottom_token_replace else None,
            ...
            if is_first:
                prev_bottom_cache = bottom_cache
            ...
            for j, ((cam_idx, time, camera), edited_image_pil) in enumerate(zip(windows, edited_images)):
                ...
                uids = {cam.uid for cam in train_cameras}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10000, 14000, 20000, 24000, 29000, 30000, 31000, 34000, 40000, 44000, 50000, 60000])
