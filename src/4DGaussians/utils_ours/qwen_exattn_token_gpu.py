...
from utils_ours.flow_check import forward_backward_consistency_check
...
def backward_warp(latent_hw, flow_t2p):
    """
    latent_hw: [B,C,H,W]  (to warp features)
...
class QwenDoubleStreamProcessor:
    ...
    def __call__(
        ...
        right_cache: Optional[Dict[str, torch.Tensor]] = None,
        bottom_cache: Optional[Dict[str, torch.Tensor]] = None,
...
class QwenDoubleStreamExtendedAttnProcessor:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if (t_idx is not None) and (prev_right_cache is not None or prev_bottom_cache is not None):
            device = hidden_states.device
            dtype  = hidden_states.dtype

            batch_idx = [2, 3]

            if bottom_token_replace:
                mode = "bottom"
                overlap_idx = [0, 2]
                layer_bucket = _pick_pivot_layer_bucket(prev_bottom_cache, self.layer_index, t_idx)
                if not layer_bucket:
                    raise ValueError("No pivot cache found")
                else:
                    pivot_attn = layer_bucket.get("attn", None)
            else:
                mode = "right"
                overlap_idx = [0, 1]

                layer_bucket = _pick_pivot_layer_bucket(prev_right_cache, self.layer_index, t_idx)
                if not layer_bucket:
                    raise ValueError("No pivot cache found")
                else:
                    pivot_attn = layer_bucket.get("attn", None)

            if (pivot_attn is not None and (mode == "right")):
                target_attn_list = []
                ...
                for fwd, bwd in zip(flows_fwd, flows_bwd):
                    ...
                    if (flow_f is not None) and (flow_b is not None):
                        hard_mask, _ = forward_backward_consistency_check(flow_f, flow_b)
                    else:
                        soft_mask = None
                ...
                # per pivot stream
                for p_idx, pa in enumerate(pivot_attn):
                    ...
                    for c_idx, (ch_pivot, ch_base) in enumerate(zip(pivot_chunks, base_chunks)):
                        ...
                        # soft_mask = flow_soft_mask_list[p_idx]
                        ...
                        if flow_f is not None:
                            ...
                            #     m = soft_mask.unsqueeze(1).to(p_warp.dtype)
            ...
            for i, prev_pa in enumerate(pivot_attn):
        ...
        if t_idx is not None:
            bucket = right_cache.setdefault(int(t_idx), {})
            right_attn = torch.stack([img_attn_output[2], img_attn_output[3]], dim=0)
            bucket[self.layer_index] = {
                'attn': right_attn.detach().to('cuda'),
            }
        
        if is_cache_bottom and t_idx is not None:
            bucket = bottom_cache.setdefault(int(t_idx), {})
            bottom_attn = torch.stack([img_attn_output[1], img_attn_output[3]], dim=0)
            bucket[self.layer_index] = {
                'attn': bottom_attn.detach().to('cuda'),
            }
